...
function Tile({ title, body, children, className = "" }: { title: string; body: string; children: ReactNode; className?: string }) {
  return (
    <motion.div variants={fadeUp} className={`flex flex-col overflow-hidden border border-border bg-surface ${className}`}>
      <div className="flex flex-1 items-center justify-center bg-surface-alt/60 p-6">{children}</div>
      <div className="p-6">
        <h3 className="font-serif-ed text-lg font-semibold tracking-tight">{title}</h3>
    ...
    <div className="w-full max-w-[260px] rounded-xl border border-border bg-surface p-2.5 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="h-14 rounded-lg bg-surface-alt p-1.5">
          <motion.div
            className="h-5 rounded-md bg-[var(--mark)]"
  ...
  return (
    <div className="flex h-28 w-full items-center justify-center rounded-xl border-2 border-dashed border-border">
      <motion.div
        ...
        whileDrag={{ scale: 1.08, rotate: 3, boxShadow: "0 20px 40px -12px rgba(26,24,20,0.35)" }}
...
const TEAM = [
  { name: "Ana Torres", color: "var(--mark)" },
  { name: "Luis Mora", color: "#8c6d1f" },
  { name: "Cuenta Demo", color: "#1a1814" },
  { name: "Sara Gil", color: "#3d5a4c" },
  { name: "Leo Paz", color: "#5b564c" },
];
    ...
    <div className="flex -space-x-3 transition-all duration-300 hover:space-x-1 [&>*]:transition-[margin] [&>*]:duration-300">
      {TEAM.map((p, i) => (
        <motion.div key={p.name} whileHover={{ y: -6, zIndex: 10 }} className="relative" style={{ zIndex: TEAM.length - i }}>
          ...
          <span className="absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-surface bg-[var(--mark)]" />
        </motion.div>
      ))}
    </div>
  );
...
function CommentVisual() {
  ...
  return (
    <div className="w-full max-w-[250px] space-y-2.5">
      ...
      <div className="ml-auto flex w-fit items-center gap-1 rounded-xl rounded-br-sm bg-ink px-3.5 py-3">
        {[0, 1, 2].map((i) => (
          <motion.span
            key={i}
  ...
  return (
    ...
    >
      <motion.span
        ...
        className={`flex h-9 w-9 items-center justify-center rounded-full text-white shadow-md ${dark ? "ml-auto bg-ink" : "bg-[var(--mark)]"}`}
      >
  ...
  return (
    <motion.div whileHover="hover" className="flex flex-col items-center gap-3">
      <motion.span
        ...
        className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--mark)]/10 text-[var(--mark)]"
      >
